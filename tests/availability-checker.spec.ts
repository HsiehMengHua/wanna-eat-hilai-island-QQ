...
describe('Booking Availability Analysis', () => {
    it('3 dates have time slots but only 2 dates are available for 2 persons', () => {
        const bookingSize = 2;
        const checker = new AvailabilityCheckerForTest('doesnt-matter-in-test', bookingSize);
        ...
        const result = checker.analyzeResponse(response);
...
class AvailabilityCheckerForTest extends AvailabilityChecker {
    analyzeResponse(response: BookingCapacityResponse): DayAvailability[] {
        return super.analyzeResponse(response);
    }
}